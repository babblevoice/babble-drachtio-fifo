...
class fifo {
  ...
  */
  queue( options ) {
    ...
    options.call.vars.fifo.name = this._options.name
  ...
  */
  _fifoupdate( qc ) {
    ...
    for( let i = qc.call.vars.fifo.position; 
      ...
      let otherqcuuid = this._fifos[ qc.priority - 1 ][ i ]
      let otherqc = this._calls.get( otherqcuuid )
      if( otherqc ) {
        otherqc.call.vars.fifo.position = i
        otherqc.call.emit( "fifo.update" )
      }
    }
  }
  ...
  */
  async _ringallearly( agentcall ) {
    ...
    agentcall.on( "call.destroyed", ( call ) => {
      agent.callcount--
      if( agent.callcount < 0 ) agent.callcount = 0
    } )
  }
  ...
  */
  _ringallconfirm( agentcall ) {
    let qc = this._removefromfifo( agentcall.parent )
    if( !qc ) return
  ...
  _enterpriseallconfirm( agentcall ) {
    ...
    if( 0 === this._callcount ) {
      for( const agentcall of this._enterpriseoutboundcalls ) {
        agentcall.hangup( entcall.hangupcodes.LOSE_RACE )
      }
      this._enterpriseoutboundcalls.clear()
    }

    let queuedcall = this._getnextcaller( true )
  ...
  */
  async _enterprise() {
    ...
    let agents = Object.values( this._agents ).sort( ( a, b ) => { return a.last - b.last } )
