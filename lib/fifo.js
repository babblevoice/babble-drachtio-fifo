  ...
  _enterpriseallconfirm( agentcall ) {
    ...
    const request = { queuedcall: queuedcall.call, agentcall, complete: ( adopt = false ) => {
      ...
      if( adopt && !request.queuedcall.destroyed ) {
        ...
        let options = {}
        options.remote = {}
        options.remote.display = queuedcall.call.remote.name
        options.remote.username = queuedcall.call.remote.user
        options.remote.realm = queuedcall.call.remote.host

        agentcall.update( options )
      }
    } }
