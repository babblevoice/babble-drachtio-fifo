...
class fifo {
  ...
  */
  async queue( options ) {
    ...
    */
    const retval = await qc.wait()

    if( "timeout" === retval ) {
      this.#hangupringallchildren( options.call, options.call.hangupcodes.REQUEST_TIMEOUT )
    }
   ...
   * @returns 
   */
  #hangupringallchildren( call, reason ) {

    if( "ringall" !== call.vars.fifo.mode ) return
  ...
  */
  ...
  getcallbycallerid( callerid ) {
    for ( let priority = 0; priority < this._fifos.length; priority++ ) {
      for ( let index = 0; index < this._fifos[ priority ].length; index++ ) {
  ...
  */
   ...
   */
  getoldestcall() {
    const qc = this._getnextcaller()
    if( qc ) return qc._call
  }
  ...
  */
  _pick( call ) {
    ...
    if ( !qc ) {
      /* Not in our queue any more, so an agent answering has already popped
         this caller out in prebridge and bonded to them. They have not been
         mixed yet - that happens in confirm - so the picker and the agent are
         both about to land on the same caller. Give the agent leg up: the
         pick has already committed on the caller's side. SIP-227. */
      this.#releaseclaimedagentcalls( call )
      return
    }
    ...
    if( "ringall" == this._mode ) {
      this.#hangupringallchildren( call, call.hangupcodes.PICKED_OFF )
    } else {
  ...
  */
  ...
  /**
   * Release any agent leg that has claimed this caller in prebridge but has
   * not yet been mixed with them. Between prebridge and confirm the caller is
   * out of the queue yet bond() has marked nothing on the caller itself - no
   * parent, no child, no picked flag - so the caller looks free to everybody
   * else while an agent is already committed to them. If somebody picks in
   * that window the agent has to lose, otherwise confirm adopts them on top
   * of the picker and the caller is mixed with two legs. SIP-227.
   *
   * @param { object } call - the caller that has just been picked
   * @private
   */
  #releaseclaimedagentcalls( call ) {
    for( const agentcall of this._enterpriseoutboundcalls ) {
      if( !agentcall.vars ) continue
      if( call.uuid !== agentcall.vars.fifoclaimed ) continue
      agentcall.vars.fifoclaimed = undefined
      /* the hangup may not have taken by the time confirm runs, so say so
         explicitly - confirm must not go on to adopt this caller */
      agentcall.vars.fifolostrace = true
      agentcall.hangup( agentcall.hangupcodes.LOSE_RACE )
    }
  }

  /**
   * Clear surplus agent legs.
   *
   * Enterprise agent legs are dialled with "orphan": true, so a ringing leg is
   * not a child of any caller - it is only tied to one at prebridge, when it
   * answers. So when a caller leaves the queue (picked by an intercept, or
   * hung up) the legs dialled for them are still ringing and nothing in the
   * call graph connects the two. Leave a surplus leg up and it answers a
   * moment later onto a caller who is already talking to somebody else: a
   * crossed call.
   *
   * We keep one in-flight leg per caller still waiting and hang up the rest.
   * A leg that has answered is left alone here - callmanager marks a leg
   * established before it runs prebridge, so an established leg either has a
   * caller already or is about to be given one, and #releaseclaimedagentcalls
   * deals with it if that caller gets picked away.
   *
   * @param { object } reason - hangup code for the legs we release
   * @param { object } [ keep ] - a leg to leave alone (the one being bridged)
   * @private
   */
  #clearenterpriseagentcalls( reason, keep ) {

    const inflight = []
    for( const agentcall of this._enterpriseoutboundcalls ) {
      if( agentcall === keep ) continue
      if( agentcall.established ) continue
      inflight.push( agentcall )
    }

    /* one leg per waiting caller, the remainder are surplus */
    let surplus = inflight.length - this._callcount
    if( 0 >= surplus ) return
  ...
  */
  async _ringallearly( agentcall ) {
    ...
    agentcall.vars.fifoagententity = entity.uri
    agentcall.vars.fromfifo = this._name
    ...
    if( Array.isArray( agents ) ) {
      agents.forEach( ( agent ) => {
        ...
        const options = {
          ...
          "uactimeoutreason": "SIP;cause=200;text=\"Call still queued\"",
          "entity": {
            "uri": agent.uri,
            "max": 1
          }
  ...
  */
  _enterpriseallconfirm( agentcall, queuedcall ) {
    ...
    const request = { queuedcall: queuedcall.call, agentcall, complete: ( adopt = false ) => {
      ...
      /* the caller was picked out from under us between prebridge and here,
         so we have already been released - do not mix in on top of whoever
         took them */
      if( agentcall.vars && agentcall.vars.fifolostrace ) return

      /* the claim window closes here - we are about to be mixed */
      if( agentcall.vars ) agentcall.vars.fifoclaimed = undefined
  ...
  */
  _enterpriseallprebridge( agentcall ) {
    ...
    const agent = this._agents[ agentcall.vars.fifoagententity ]
    if( 0 === agent.callcount ) {
      agent.state = "busy"
    }

    this.#clearenterpriseagentcalls( agentcall.hangupcodes.LOSE_RACE, agentcall )

    const queuedcall = this._getnextcaller( true )
    if( !queuedcall ) {
      /* nothing left to hand this leg - it is orphaned, so releasing it here
         is the only thing stopping it landing on somebody else's caller */
      agentcall.hangup( agentcall.hangupcodes.LOSE_RACE )
      return
    }

    /* We own this caller from here until confirm mixes us with them. Nothing
       is set on the caller to say so, so record it on our side - it is what
       lets a pick arriving in this window find and release us. SIP-227. */
    agentcall.vars.fifoclaimed = queuedcall.call.uuid
  ...
  */
  addagent( agent, agentinfo ) {
    if( !this.hasagent( agent ) ) {
      ...
      this._options.em.emit( "fifo.agent.add", { "name": this._name, "domain": this._domain.name, agent } )
      return true
    }
    return false
  }
  ...
  */
  deleteagent( agent ) {
    if( this.hasagent( agent ) ) {
      ...
      this._options.em.emit( "fifo.agent.remove", { "name": this._name, "domain": this._domain.name, agent } )
    }
  }
