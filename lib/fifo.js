...
class fifo {
  ...
  */
  _onqueuetimeout( qc ) {
    ...
    if( nextqc.uuid === qc._call.uuid ) isnextcaller = true
  ...
  */
  _hangup( call ) {
    let nextqc = this._getnextcaller()
    if( nextqc.uuid === call.uuid ) {
      /* hangup current ringing agents */
    }
  ...
  @private
  */
  _ringallearly( agentcall ) {
    this._ringingagents.push( agentcall )
  }

  _ringallfail( agentcall ) {
    this._agentfailcount++
    if( this._agentfailcount >= this._ringingagents.length ) {
      this._ringingagents = []
  ...
  async _ringall() {
    ...
    for( let agent of this._agents ) {
      ...
      nextcall._call.newuac( options, {
        ...
        "fail": this._ringallfail.bind( this ), 
  ...
  */
  async _enterprise() {
    ...
    if( this._ringingagents.length > this._callcount  ) return
    if( 0 === this._callcount ) return
  ...
  @param { Array< string > } agents - array of agents i.e. [ "1000@dummy.com" ]
  */
  agents( agents ) {
    this._agents = agents
  }
