...
class fifo {
  ...
  */
  queue( options ) {
    ...
    if ( options.positionstalecall ) {
      options.positionstalecall.noremoveonhangup = false
      this._hangup( options.positionstalecall )
    }
  ...
  */
  getcallbycallerid( callerid ) {
    for ( var i = 0; i < this._fifos.length; i++ ) {
      ...
      if ( othercall
          && othercall.call.callerid == othercall.call.callerid ) {
        othercall.call.__position = i
