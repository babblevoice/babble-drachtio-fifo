...
class fifo {
  ...
  */
  ...
  @param { object } callback function for handling call's callback
  ...
  */
  queue( options, callback ) {
    ...
    for ( var i = 0; i < this._fifos.length; i++ ) {
      const othercall = this._calls.get( this._fifos[i][0] )
      if ( othercall && othercall.call.callerid == othercall.call.callerid ) {
        exists = true
        break
      }
    }

    if ( exists && !callback( options.call ) ) {
      return options.call.hangup( options.call.hangupcodes.LOSE_RACE )
    }
  ...
  */
  _pick( call ) {
    ...
    call.on( "call.destroyed", cleanupcall )
