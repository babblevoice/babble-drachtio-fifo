...
class fifo {
  ...
  */
  _hangup( call ) {
    ...
    if( nextqc.uuid === call.uuid ) isnextcaller = true
  ...
  */
  async _ringall() {
    ...
    for( let agent of this._agents ) {
      this._ringingagents = this._ringingagents.concat( this._callagent( agent ) )
    }
    ...
    for( dialogpromise of this._ringingagents ) {
      if( failure || dialog.callId !== dialogpromise.invitereq.get( "call-id" ) ) {
    ...
    if( "timeout" === failure ) clearTimeout( timerid )
