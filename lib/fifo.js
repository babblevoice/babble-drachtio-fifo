...
class fifo {
  ...
  */
   ...
   * A leg that is already established is on a call of its own and is never
   * touched. One that is mid-answer is fair game - that is precisely the leg
   * about to cross - but we give it up last, so we only drop an answering
   * agent when there is nothing cheaper to release.
   *
  ...
  #clearenterpriseagentcalls( reason, keep ) {
    ...
    const establishing = ( c ) => { return c.state && c.state.establishing ? 1 : 0 }
    inflight.sort( ( a, b ) => { return establishing( a ) - establishing( b ) } )
