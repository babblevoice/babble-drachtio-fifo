...
class domain {
  ...
  */
  getcallbyuuid( options ) {
    ...
    if ( !options.name ) {
      for( const [ ,fifo ] of this._fifos ) {
        ...
        const call = f.getcallbyuuid( options.uuid )
        if ( call )
          return call
      }
      return
    ...
    return f.getcallbyuuid( options.uuid )
  }
   ...
   * @param { object } options
   * @param { string } options.name - the name of the queue
   * @param { object } options.call - the call object
   */
  getcallbycallerid( options ) {
    const f = this.getfifo( options.name )
    return f.getcallbycallerid( options.call.callerid )
  }
